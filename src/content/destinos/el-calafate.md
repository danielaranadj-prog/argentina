---
title: "El Calafate"
img: "/uploads/calafate.jpg"
desc: "Donde el hielo ruge y la naturaleza manda."
historia: "Abrigate bien, che, porque estás entrando a una de las postales más imponentes del planeta. Aquí uno se siente chiquitito de verdad. Tener enfrente al Perito Moreno te cambia la perspectiva de la vida: una pared de 60 metros que avanza y se rompe frente a tus ojos con un ruido similar a un trueno. No venís solo a ver, venís a escuchar cómo cruje la tierra."
secciones:
  - icon: "fa-calendar-alt"
    label: "Tiempo Sugerido"
    text: "3 días completos. Uno para pasarelas, otro para navegación y uno extra por clima."
  - icon: "fa-wind"
    label: "Clima"
    text: "Ventoso y frío. El clima cambia en 10 minutos. Vestite por capas (cebolla)."
  - icon: "fa-wallet"
    label: "Presupuesto"
    text: "Medio-Alto. Las excursiones (navegación/trekking) son lo más costoso del viaje."
  - icon: "fa-bus"
    label: "Transporte"
    text: "No hay transporte público al glaciar. Necesitás contratar transfer o alquilar auto."
tours:
  - titulo: "Minitrekking Perito Moreno"
    precio: "US$ 250"
    duracion: "Día completo"
    link: "#"
    imagen: "https://images.unsplash.com/photo-1518182170546-0766ce6fec56?auto=format&fit=crop&w=600&q=80"
  - titulo: "Navegación Todo Glaciares"
    precio: "US$ 180"
    duracion: "Día completo"
    link: "#"
    imagen: "https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?auto=format&fit=crop&w=600&q=80"
  - titulo: "Estancia Cristina 4x4"
    precio: "US$ 150"
    duracion: "Día completo"
    link: "#"
    imagen: "https://images.unsplash.com/photo-1476514525535-07fb3b4ae5f1?auto=format&fit=crop&w=600&q=80"
faqs:
  - pregunta: "¿Cuál es la mejor época para visitar El Calafate?"
    respuesta: "De octubre a abril (primavera-verano). Días más largos y temperaturas menos extremas. Julio-agosto es temporada de nieve, lindo pero muy frío (-10°C)."
  - pregunta: "¿Puedo ir al glaciar por mi cuenta?"
    respuesta: "Podés ir en auto alquilado, pero las excursiones sobre el hielo (Minitrekking, Big Ice) requieren reserva con operadores autorizados. Las pasarelas se visitan por cuenta propia."
  - pregunta: "¿Con cuánta anticipación reservo el Minitrekking?"
    respuesta: "Mínimo 2-3 meses en temporada alta. Es LA excursión más demandada de Argentina y tiene cupos limitados por día. No te arriesgues a quedarte afuera."
  - pregunta: "¿Qué pasa si llueve el día de mi excursión?"
    respuesta: "El clima patagónico es impredecible. Las excursiones se suspenden con viento fuerte o nevadas intensas. Por eso siempre recomendamos tener un día extra de margen."
  - pregunta: "¿Cuánto cuesta la entrada al Parque Nacional?"
    respuesta: "Es en pesos argentinos y cambia cada año. Se paga aparte de las excursiones. Llevá efectivo porque a veces los posnet fallan. No aceptan dólares."
leadMagnet:
  titulo: "Checklist para la Patagonia Glaciar"
  descripcion: "No sufras el frío: llevá exactamente lo que necesitás. Incluye lista de ropa por capas, accesorios y cosas que la gente olvida."
  archivo: "/downloads/checklist-patagonia-glaciar.pdf"
  imagen: "https://images.unsplash.com/photo-1518182170546-0766ce6fec56?auto=format&fit=crop&w=600&q=80"
---
### 🧊 Guía para Domar al Gigante de Hielo

El Calafate es hermoso pero hostil si no vas preparado. Aquí los secretos para que no congeles tu presupuesto (ni tus dedos).

#### 1. La Regla de las Capas
Nunca salgas del hotel sin una campera, aunque haya sol.
* **Capa 1:** Térmica (pegada al cuerpo).
* **Capa 2:** Polar o buzo (abrigo).
* **Capa 3:** Rompevientos impermeable (fundamental por la lluvia y el viento).

#### 2. Cómo llegar al Glaciar Perito Moreno
El glaciar está a 80km del pueblo.
* **Transfer:** Lo más común. Te pasan a buscar por el hotel.
* **Auto:** Si alquilás, manejá con cuidado. La ruta es asfaltada pero puede haber hielo negro en invierno.
* **Entrada al Parque:** Se paga aparte y **solo aceptan efectivo** (pesos argentinos) o tarjeta de débito si hay señal (no te confíes).

#### 3. Gastronomía Patagónica
No te podés ir sin probar el **Cordero al Asador**. Los mejores lugares suelen estar sobre la Avenida del Libertador.
* **El Postre:** Pedí algo con **Calafate** (es un fruto rojo silvestre). La leyenda dice que quien lo come, siempre vuelve a la Patagonia.

> **Dato Clave:** Si vas a hacer el Minitrekking (caminar sobre el hielo), reservalo con **meses de anticipación**. Es la excursión más solicitada de Argentina y se agota siempre.